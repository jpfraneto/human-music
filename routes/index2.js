...
router.get("/", (req, res) => {
    ...
    .then((presentDay)=>{
        res.render("eternity", {
            ...
            presentRecommendation : presentDay.recommendationsOfThisDay[presentDay.elapsedRecommendations],
            today : todayDay
        });
    });
});
...
router.get("/checkSystemStatus", (req,res) => {
    ...
    .then((presentDay) => {
        ...
        if(presentDay){
            ...
            if(presentDay.systemStatus === "recommendation"){
                ...
                answer.nasaInfo = presentDay.apod;
    ...
    .catch(()=>{console.log("There was an error getting the present day in the check system status function")})
});
...
router.post("/getRecommendationInformation", (req,res)=>{
    let answer = {};
    Recommendation.findOne({youtubeID: req.body.youtubeID})
    .then((foundRecommendation)=>{
        if(req.user){
            if(req.user.favoriteRecommendations){
                let indexOfRecommendation = req.user.favoriteRecommendations.indexOf(foundRecommendation._id);
        ...
        answer.recommendation = foundRecommendation;
        console.log(answer);
        res.json(answer);
    })
})
...
router.post("/favorited", (req, res) => {
    if(req.body.recommendationID){
        ...
        .then((recommendation)=>{
            req.user.favoriteRecommendations.push(recommendation._id);
            req.user.save(()=>{
        ...
        .then((presentRecommendation) => {
            if(req.user){
            req.user.favoriteRecommendations.push(presentRecommendation._id);
            req.user.save(()=>{
                console.log("The recommendation was added to the user");
            });
            } else {
...
router.post("/unfavorited", (req, res) => {
    if (req.body.recommendationID){
        ...
        .then((recommendation)=> {
            ...
            req.user.save(()=>{
                console.log("The recommendation was deleted from the user");
            })
        })
        .catch((error)=>{
            console.log(error)
...
router.post("/nextRecommendationInfo", (req, res) => {
    let videoID = req.body.youtubeID;
    let daySKU = req.body.daySKU;
    let response = {};

    Day.findOne({daySKU : daySKU}).populate("recommendationsOfThisDay")
    .then((foundDay)=>{
        response.apod = foundDay.apod;
        let j = 0
        for (let i=0; i<foundDay.totalRecommendationsOfThisDay; i++){
            if (foundDay.recommendationsOfThisDay[i].youtubeID === videoID){
                break
            }
            j++
        }
        if (j === foundDay.totalRecommendationsOfThisDay){
            response.nextRecommendation = foundDay.recommendationsOfThisDay[0]
        } else {
            response.nextRecommendation = foundDay.recommendationsOfThisDay[j+1]
        }
       res.json(response)
    })
    // .catch(err => console.log("There was an error in the nextRecommendationInfo function!"))
});

router.post("/pastRecommendation", (req, res) => {
    let timestamp = req.body.timestamp;
    let daySKU = req.body.daySKU;
    Day.findOne({daySKU : daySKU}).populate("recommendationsOfThisDay")
    .then((thisDay)=>{
        for (i=0;i<thisDay.totalRecommendationsOfThisDay;i++){
            if (thisDay.startingTimestampsOfThisDay[i] < timestamp){
                if (i === thisDay.totalRecommendationsOfThisDay){
                    response = {
                        recommendation : thisDay.recommendationsOfThisDay[i], 
                        elapsedSeconds : Math.round((timestamp - thisDay.startingTimestampsOfThisDay[i])/1000),
                    }
                } else if (timestamp < thisDay.startingTimestampsOfThisDay[i+1]){
                    response = {
                        recommendation : thisDay.recommendationsOfThisDay[i], 
                        elapsedSeconds : Math.round((timestamp - thisDay.startingTimestampsOfThisDay[i])/1000),
                    }
    ...
    // .catch(console.log("There was an error in the pastRecommendation route"))
    // Recommendation.find({type:"music"})
    // .then((foundRecommendations)=>{
    //     foundRecommendations.forEach((recommendation)=>{
    //         if(videoID === recommendation.url.slice(-11)){
...
router.get("/days", function(req, res){
 ...
 router.get("/newpast", function(req, res){
    Day.findOne({daySKU : todayDay}).populate("recommendationsOfThisDay")
    .then((foundDay)=>{
        let totalRecommendations = foundDay.totalRecommendationsOfThisDay;
        let recommendationIDS = "";
        foundDay.recommendationsOfThisDay.forEach((recommendation)=>{
            recommendationIDS += recommendation.youtubeID + "?";
        })
        randomRecommendation = foundDay.recommendationsOfThisDay[Math.floor(Math.random() * totalRecommendations)]
        res.render("newPast", {
            elapsedSeconds:0, 
            presentRecommendation: randomRecommendation, 
            today : todayDay,
            thisDay : foundDay,
            startingTimestamp : foundDay.startingDayTimestamp,
            endingTimestamp : foundDay.startingDayTimestamp + 86400000,
            recommendationIDS : recommendationIDS
        });
    })
    // .catch(()=>{
    //     console.log("There was an error displaying the newpast page")
    // })
 });
