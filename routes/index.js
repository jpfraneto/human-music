...
router.get('/randomRecommendation', (req, res) => {
  Recommendation.find({ status: 'past' })
    .exec()
    .then(allPastRecommendations => {
      let randomIndex = Math.floor(
        Math.random() * allPastRecommendations.length
      );
      ...
      Cycle.find({})
        ...
        .then(foundCycles => {
          ...
          if (allPastRecommendations) {
            ...
            answer.message =
              'There are ' +
              allPastRecommendations.length +
              ' recommendations in the past and this one was chosen from there.';
          } else {
            answer.pastRecommendations = [];
            answer.message =
              'So bad, there are no recommendations in the past.';
          }
          res.json(answer);
        });
    });
    ...
    .then(foundUser => {
      if (req.body.userQuery === 'favorites') {
        if (foundUser.favoriteRecommendations.length > 0) {
          ...
          answer.message =
            'There are ' +
            foundUser.favoriteRecommendations.length +
            ' recommendations in the past and this one was chosen from there.';
        } else {
        ...
        if (foundUser.recommendations.length > 0) {
          ...
          answer.message =
            'There are ' +
            foundUser.recommendations.length +
            ' recommendations in the past and this one was chosen from there.';
        } else {
          answer.userRecommendations = [];
          answer.message = 'So bad, there are no recommendations in this user.';
...
router.post('/getRecommendationInformation', (req, res) => {
  ...
  Recommendation.findOne({ youtubeID: req.body.recommendationID })
    ...
    .then(queriedRecommendation => {
      ...
      if (req.user) {
        if (req.user.favoriteRecommendations) {
          let indexOfRecommendation = req.user.favoriteRecommendations.indexOf(
            queriedRecommendation._id
          );
          if (indexOfRecommendation === -1) {
            answer.isFavorited = false;
          } else {
...
router.post('/unfavorited', (req, res) => {
  ...
  if (req.user) {
    Recommendation.findOne({ youtubeID: req.body.recommendationID })
      .exec()
      .then(thisRecommendation => {
        const index = req.user.favoriteRecommendations.indexOf(
          thisRecommendation._id
        );
        if (index > -1) {
        ...
        req.user.save(() => {
          console.log(
            'Updated the user after deleting the recommendation ' +
...
router.post('/checkIfRepeated', (req, res) => {
  ...
  Recommendation.findOne({ youtubeID: req.body.videoID })
    ...
    .catch(err =>
      console.log('There was an error checking if the video is repeated!')
    );
});

router.get('/reviewer', (req, res) => {
  Recommendation.findOne({ reviewed: false }).then(recommendationForReview => {
    if (recommendationForReview) {
      res.render('reviewer', {
        ...
        recommendation: {
          ...
          recommenderName:
            recommendationForReview.author.name ||
            recommendationForReview.author.username,
      ...
      console.log(
        'There are not any more recommendations that need to be reviewed, you are going to be redirected to the present'
      );
      res.redirect('/');
    }
  });
...
router.post('/reviewer', (req, res) => {
  if (req.body.password === process.env.REVIEWER_PASS) {
    Recommendation.findById(req.body.recommendationID).then(
      foundRecommendation => {
        ...
        if (req.body.reviewerRadioBtn === 'true') {
          foundRecommendation.save(() => {
            console.log(
              'The recommendation ' +
                foundRecommendation.name +
                ' was reviewed and sent to the future'
            );
          ...
          foundRecommendation.save(() => {
            console.log(
              'The recommendation ' +
                foundRecommendation.name +
                " doesn't work and was sent to the void"
            );
            res.redirect('/reviewer');
          });
        }
...
router.post('/register', async function (req, res, next) {
  try {
    ...
    middlewareObj.sendVerificationEmail(
      registeredUser.username,
      registeredUser.email,
      registeredUser.activeToken
    );
    registeredUser.save(() => {
  ...
  User.findOne({ email: req.body.email }).then(foundUser => {
    if (foundUser.passResetString === req.body.resetCode) {
      if (foundUser.passResetExpires > nowTimestamp) {
        answer.status = true;
        answer.message =
          'All OK with this account, proceed to update the password';
      } else {
        answer.status = false;
        answer.message =
          'Your code for creating a new password has expired, please click the following link to get a new one.';
      }
    } else {
      answer.status = false;
      answer.message =
        "I'm sorry, but the email you provided does not match the one in our end. Try again, or click the following link to get a new code.";
    }
  });
});
...
router.post('/pass_reset', async function (req, res) {
  ...
  if (middlewareObj.validateEmail(req.body.passReset)) {
    User.findOne({ email: req.body.passReset }).then(foundUser => {
      foundUser.passResetString = randomCode;
      foundUser.passResetExpires = new Date().getTime() + 7200000;
      foundUser.save(async () => {
        middlewareObj.sendResetEmail(
          foundUser.username,
          foundUser.email,
          randomCode
        );
        answer.message =
          'The email with the link for resetting the password was sent!';
        res.json(answer);
      });
    });
  } else {
    User.findOne({ username: req.body.passReset }).then(foundUser => {
      foundUser.passResetString = randomCode;
      foundUser.passResetExpires = new Date().getTime() + 7200000;
      foundUser.save(async () => {
        console.log(
          'The user was saved with the string that resets the password'
        );
        middlewareObj.sendResetEmail(
          foundUser.username,
          foundUser.email,
...
router.post('/addNewCommentToRecommendation', (req, res) => {
  ...
  Recommendation.findOne({ youtubeID: req.body.recommendationID }).then(
    thisRecommendation => {
      ...
      thisRecommendation
        ...
        .then(() => {
          console.log(
            'The comment by ' +
              newComment.author +
              ' was saved to this recommendation'
          );
          res.json({
            success: true,
            message: 'Comment added to the recommendation',
          });
        })
        .catch(error => {
          ...
          res.json({
            success: false,
            message: 'Comment NOT added to the recommendation',
          });
        });
    }
  );
...
router.post('/sendRecommendationToTheVoid', (req, res) => {
  Recommendation.findById(req.body.recommendationForVoid).then(
    thisRecommendation => {
      ...
      thisRecommendation.save(() => {
        res.json({
          message:
            'The recommendation ' +
            thisRecommendation.name +
            'was successfully sent to the void',
        });
      });
    }
  );
